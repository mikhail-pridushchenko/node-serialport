...
SerialPort.prototype.open = function(callback) {
  ...
  SerialPortBinding.open(this.path, this.options, function(err, fd) {
    ...
    if (process.platform !== 'win32') {
      ...
      this.serialPoller.SetLoggerCallback(function(data) {
        debug(`SerialPort ${this.path} reports: ${JSON.stringify(data.toString())}`);
      }.bind(this));
...
if (process.platform !== 'win32') {
  SerialPort.prototype._read = function() {
    ...
    if (!this.pool || this.pool.length - this.pool.used < kMinPoolSpace) {
      ...
      this.pool.used = 0;
      debug('Serialport.prototype._read: allocated new buffer.');
    }
    ...
    var _afterRead = function(err, bytesRead, readPool, bytesRequested) {
      ...
      if (err) {
        if (err.code && err.code === 'EAGAIN') {
          if (this.isOpen()) {
	    debug(`Serialport.prototype._afterRead: re-starting poller for ${this.path}: EAGAIN`);
            this.serialPoller.start();
          }
      ...
      if (bytesRead === 0) {
        if (this.isOpen()) {
	  debug(`Serialport.prototype._afterRead: re-starting poller for path ${this.path}: bytesRead == 0`);
          this.serialPoller.start();
        }
      } else {
        var b = this.pool.slice(start, start + bytesRead);
	debug(`Serialport.prototype._afterRead: have data from ${this.path}: ${JSON.stringify(b.toString())}`);

        // do not emit events if the stream is paused
        if (this.paused) {
	  debug(`Serialport.prototype._afterRead: port ${this.path} paused, not emitting`);
          if (!this.buffer) {
  ...
  SerialPort.prototype._emitData = function(data) {
    debug(`SerialPort ${this.path} emits data ${JSON.stringify(data.toString())}`);
    this.options.dataCallback(data);
  };

  SerialPort.prototype.pause = function() {
    debug(`SerialPort for path ${this.path} is paused`);
    this.paused = true;
  };

  SerialPort.prototype.resume = function() {
    debug(`SerialPort for path ${this.path} resumed`);
    this.paused = false;
