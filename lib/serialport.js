  ...
  SerialPortBinding.open(this.path, this.options, function(err, fd) {
    ...
    if (process.platform !== 'win32') {
      ...
      this.serialPoller.SetLoggerCallback(function(data) {
        debug(`SerialPort ${this.path} reports: ${data.toString('hex')}`);
      }.bind(this));
...
if (process.platform !== 'win32') {
  SerialPort.prototype._read = function() {
	...
	debug(`Serialport.prototype._afterRead: have data from ${this.path}: ${b.toString('hex')}`);
  ...
  SerialPort.prototype._emitData = function(data) {
    debug(`SerialPort ${this.path} emits data ${data.toString('hex')}`);
