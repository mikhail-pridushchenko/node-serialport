...
var escStr = require('lodash').escape;
...
if (process.platform !== 'win32') {
  SerialPort.prototype._read = function() {
	...
	debug(`Serialport.prototype._afterRead: have data from ${this.path}: "${escStr(b)}"`);
  ...
  SerialPort.prototype._logData = function(data) {
    debug(`SerialPort {this.path} reports: "${escStr(data)}"`);
  };

  SerialPort.prototype._emitData = function(data) {
    debug(`SerialPort {this.path} emits data "${escStr(data.toString())}"`);
